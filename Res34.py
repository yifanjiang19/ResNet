...
print(x_image)
#x_image = tf.nn.batch_normalization(x_image)
conv_1 = conv2d(x_image,7,3,64,[64],2,name='conv_1')
print(conv_1)
...
avg_pool = tf.nn.avg_pool(conv_30,ksize=[1,2,2,1],strides=[1,1,1,1],padding='SAME')

avg_pool = tf.reshape(avg_pool,[-1,512])
print(avg_pool)
fc_1 = fc_layer(avg_pool,512,1000,name='fc_1')
#fc_2 = fc_layer(fc_1,1000,100,name='fc_2')
...
with tf.Session() as sess:
    ...
    #        if i%10 == 0:
            ...
            # print (epoch,i*batch_size,loss,train_accuracy,loss_mean/(i+1),acc_mean/(i+1))
    #            print ("step:%s, accuracy:%.3f"%(i*batch_size,train_accuracy))
            if i%300 == 0:
                yb_test = map(cifar.one_hot,y_test[j*batch_size:j*batch_size+batch_size])
                test_dict =  {x:x_test[j*batch_size:j*batch_size+batch_size],
                              labels:yb_test}
                j+=1
                print(sess.run([accuracy],feed_dict=test_dict))
    #            print ("step:%s,loss:%.3f accuracy:%.3f"%(i*batch_size,sess.run([cross_entropy,accuracy],feed_dict=test_dict)))
